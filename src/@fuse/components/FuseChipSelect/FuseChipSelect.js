...
function replaceIfSubstring(original) {

    var createLabel = "Create";
    var nooptionLabel = "No Options"

    var idx = original.indexOf(createLabel);
    if (idx != -1) {
        return original.substr(createLabel.length);
    } else {
        idx = original.indexOf(nooptionLabel)
        if (idx != -1) {
            return "Sin opciones";
        }else{
            return original;
...
function Option(props)
{
    return (      
        ...
        >
            {/* {props.children} */}
            {/* {console.log( "props-chiledren ======", props.children)} */}
            {replaceIfSubstring(props.children) }
        </MenuItem>
    );
}
